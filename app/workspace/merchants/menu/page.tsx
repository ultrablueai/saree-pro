...
export default async function MenuManagementPage() {
  ...
  const merchant = (await db.get(`SELECT id, name FROM merchants WHERE owner_user_id = ?`, [session.id])) as
    | MerchantRow
    | undefined;
  ...
  async function submitMenuItem(formData: FormData) {
    'use server';
    await createMenuItem(formData);
  }

  async function removeCategory(categoryId: string, _formData: FormData) {
    'use server';
    void _formData;
    await deleteMenuCategory(categoryId);
  }

  async function toggleAvailability(itemId: string, _formData: FormData) {
    'use server';
    void _formData;
    await toggleMenuItemAvailability(itemId);
  }

  async function removeMenuItem(itemId: string, _formData: FormData) {
    'use server';
    void _formData;
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-8">
      ...
      <div className="mb-8 rounded-xl border border-[var(--color-border)] bg-white p-6">
        ...
        <form action={submitCategory} className="mt-4 flex gap-3">
          <input
            type="text"
            name="name"
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {categories.map((category) => (
              ...
              >
                ...
                <form action={removeCategory.bind(null, category.id)} className="mt-3">
      ...
      <div className="mb-8 rounded-xl border border-[var(--color-border)] bg-white p-6">
        ...
        <form action={submitMenuItem} className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-2 block text-sm font-medium text-[var(--color-ink)]">Item Name *</label>
            <input
              type="text"
              name="name"
              required
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-[var(--color-ink)]">Price *</label>
            <input
              type="number"
              name="price"
              required
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-[var(--color-ink)]">Category</label>
            <select
              name="categoryId"
              className="w-full rounded-xl border border-[var(--color-border)] bg-white px-4 py-3 text-sm outline-none transition focus:border-[var(--color-accent)] focus:ring-2 focus:ring-[rgba(214,107,66,0.18)]"
            >
          ...
          <div>
            <label className="mb-2 block text-sm font-medium text-[var(--color-ink)]">Image URL</label>
            <input
              type="url"
              name="imageUrl"
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <div className="mt-3 flex items-center gap-6 text-sm">
                      ...
                      {item.category_name && (
                        <span className="text-[var(--color-muted)]">Category: {item.category_name}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <form action={toggleAvailability.bind(null, item.id)}>
                      <Button type="submit" variant="secondary" className="text-sm">
                        {item.is_available ? 'Mark Unavailable' : 'Mark Available'}
                      </Button>
                    </form>
                    <form action={removeMenuItem.bind(null, item.id)}>
                      <Button type="submit" variant="secondary" className="text-sm">
                        Delete
                      </Button>
                    </form>
