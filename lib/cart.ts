...
export async function addToCart(
  ...
  await db.run(
    `INSERT INTO shopping_carts (user_id, merchant_id, menu_item_id, quantity, special_instructions, selected_options_json, created_at, updated_at)
...
export async function updateCartItemQuantity(
  ...
  await db.run(
    `UPDATE shopping_carts SET quantity = ?, updated_at = datetime('now') 
...
export async function removeFromCart(userId: string, menuItemId: string): Promise<void> {
  const db = await getDbExecutor();
  await db.run(
    `DELETE FROM shopping_carts WHERE user_id = ? AND menu_item_id = ?`,
    [userId, menuItemId]
  );
}
...
export async function getCartTotal(userId: string): Promise<{ subtotal: number; itemCount: number }> {
  const db = await getDbExecutor();
  const result = await db.get<{ subtotal: number; itemCount: number }>(
    `SELECT 
